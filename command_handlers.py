...
def msg_cmd(message,bot):
		...
		callback_button1 = types.InlineKeyboardButton(text = '⬅️',callback_data = f'wikicmd {message.from_user.id} 3')
...
def msg_start(message,bot):
		text = 'В этом боте ты получишь настоящую кошкожену, которую можно кормить, гладить и ещё много чего. Напиши <i><u>неко</u></i> или /neko чтобы начать своё приключение в ебанутом мире <b>Некославии</b>\n\nВсе команды можно узнать в /cmd'
		bot.send_message(message.chat.id, text)