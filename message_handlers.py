...
def msg_text(message,bot):
		...
		if cmd == 'неко':
			...
			text = '''Это ультимативный гайд по уебанской боевой системе, вдохновлённой игрой The Legend of Bumbo. В основе лежит складывание фигур по 4 в ряд
			<b>Элементы интерфейса (см рисунок):</b>
			<b>1.</b> Информация про хп твоего некочана, хп вражины и оставшиеся шаги. Главная цель - захуярить врага и не сдохнуть самому. Шаги тратятся на передвижение фигур, когда они заканчиваются ход переходит к мобам или другим игрокам. Мобы имеют такие обозначения:
			💥 – кол-во урона, которое моб нанесёт во время своего хода
			🫥 – единоразовый уворот от любой твоей атаки
			<b>2.</b> Кнопки для выбора цели, которая будет получать урон
			<b>3.</b> Игровое поле, на котором нужно складывать одинаковые фигуры по 4 в ряд. Эффекты фигур:
			🟥 🟧 – блок
			🟡 🟢 – атака
			💙 – дополнительные шаги
			⭐️ – используется в комбинациях вместо любой другой фигуры
			🦴 – без эффекта, тупо вытесняет полезные фигуры
			Также возможно сложить 5 или 6 фигур чтобы, например, нанести больше урона или получить больше шагов
			<b>4.</b> Кнопки активных навыков. Каждый такой навык можно использовать раз за ход, пассивные же навыки действуют всегда
			...
			<a href="https://i.imgur.com/OLFoA9Q.png">᠌᠌</a>
			'''
			bot.send_message(message.chat.id, text)
			bot.send_message(message.chat.id, len(text))
		elif cmd == 'покер':
				minbet = 1
				if coins < minbet*5:
