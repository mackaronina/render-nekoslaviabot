...
def msg_text(message,bot):
		...
		if data is None:
			if cmd == 'неко' or cmd == '/neko':
				...
				time.sleep(3)
				text = "Твоя заявка на гражданство Некославии была одобрена и, как гражданину, мы выдаём тебе личную кошкожену. Напиши <i><u>неко</u></i> чтобы взглянуть на неё. Помни, ключ к хорошим отношениям с твоей некодевочкой это <b>доверие 💞</b>. Его можно повысить многими способами, для начала попробуй дать некодевочке поесть командой <i><u>покормить</u></i>, скорее всего она проголодалась пока ехала к тебе. После этого советую придумать ей какую-нибудь пиздатую кличку командой <i><u>имя [текст]</u></i>"
				bot.send_message(message.chat.id, text)
				#time.sleep(3)
				#text = "Если у твоей некодевочки упадёт <b>настроение 🥰</b> – просто погладь её (<i><u>погладить</u></i>), они это любят"
				#bot.send_message(message.chat.id, text)
		...
		if event > 0 and cmd not in cmd_allowed_gulat:
			bot.send_message(message.chat.id, 'Ты гуляешь ебанат\n\n<i>Напиши <u>повтор</u> чтобы повторить сообщение с выбором если ты его проебал</i>')
			bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEKbkNlGcyWsw1T1RXhFZGTgaGzubYD_AACIA8AAg7tWEjVrCd9QwTr1jAE')
			return
		...
		if cmd == 'неко' or cmd == '/neko':
			...
			text = 'Это навыки и черты характера, которыми обладает твоя кошкожена. Некодевочки обычно не такие слабые, какими кажутся на первый взгляд, а их когти острее бритвы\n\n' + sktxt1 + '\n' + sktxt2
			if gender == 1:
				text = 'Это навыки и черты характера, которыми обладает твой кошкомуж. Некомальчики обычно не такие слабые, какими кажутся на первый взгляд, а их когти острее бритвы\n\n' + sktxt1 + '\n' + sktxt2
			...
			txt = 'В гардеробе хранятся все украшения, которые твоя кошкожена с удовольствием будет носить. Они нихуя не дают, зато прикольно выглядят. Список украшений:\n\n' + item_list[item_one][0] + '\n\n' + item_list[item_two][0]
			if gender == 1:
				txt = 'В гардеробе хранятся все украшения, которые твой кошкомуж с удовольствием будет носить. Они нихуя не дают, зато прикольно выглядят. Список украшений:\n\n' + item_list[item_one][0] + '\n\n' + item_list[item_two][0]
			...
			text = 'Это кошкожены богатейших граждан Некославии. Когда-нибудь ты станешь одним из них, если, конечно, не будешь проёбывать все деньги в казино\n\n'
			...
			if gladit_kd > 0:
				...
				txt = f'{cnam} пока не хочет чтобы её гладили\n\n<i>Осталось ждать {gkd} часов</i>'
				if gender == 1:
					txt = f'{cnam} пока не хочет чтобы его гладили\n\n<i>Осталось ждать {gkd} часов</i>'
			...
			if kormit > 0:
				...
				bot.send_message(message.chat.id, f'{cnam} пока не хочет есть\n\n<i>Осталось ждать {k} часов </i>')
				bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEFLW1iwHwF7vKClo5usceHHPCXG_sHxwACKxAAAnazWEhhnVRbfrQa8ikE')
			else:
			...
			if gulat > 0:
				...
				bot.send_message(message.chat.id, f'{cnam} пока не хочет гулять\n\n<i>Осталось ждать {g} часов </i>')
				bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEFLW1iwHwF7vKClo5usceHHPCXG_sHxwACKxAAAnazWEhhnVRbfrQa8ikE')
			else:
				...
				if d == 1:
					...
					if k == 1:
						rep = rep - 1
						text = 'Ты проснулся в луже блевоты с дичайшим похмельем. Последнее воспоминание – вы заходите в Сильпо и видите Капитана Моргана по скидке. Вчерашние события, к сожалению, остаются тайной. ' + cnam + ' теперь тебе доверяет меньше как их следствие\n\n– 1 Доверия 💞'
						...
						rep = rep + 1
						text = 'Ты проснулся в луже блевоты с дичайшим похмельем. Последнее воспоминание – вы заходите в Сильпо и видите Капитана Моргана по скидке. Вчерашние события, к сожалению, остаются тайной. ' + cnam + ' теперь тебе доверяет больше как их следствие\n\n+ 1 Доверия 💞'
						bot.send_photo(message.chat.id, photo = 'AgACAgIAAx0CZQN7rQACNE1imy-Ri_WnMfD3yi2ud0IAAToM38oAAuy7MRuYt9lIMfj5yYi-9gEBAAMCAANzAAMkBA',caption = text)
			...
			bot.send_photo(message.chat.id, photo = 'AgACAgIAAx0CZQN7rQACn-JiwJJAUjK0Czuxv3RBKiKJJ61u_wACjrwxG0oRCEoxH0CUJepbQQEAAwIAA3MAAykE',caption = text)
				...
				if gender == 0:
					...
					bot.send_message(message.chat.id, text)
					bot.send_message(message.chat.id, 'Видимо, придётся выдать тебе новую кошкожену')
					...
					bot.send_message(message.chat.id, text)
					bot.send_message(message.chat.id, 'Видимо, придётся выдать тебе нового кошкомужа')
			...
			if licension < 0:
				...
				text = 'Мент пристально посмотрел сначала на лицензию, а потом на некодевочку. <b>"Ладно, всё в полном порядке. Извините за беспокойство"</b> – было сказано с некоторым разочарованием. ' + cnam + ' весь оставшийся день расспрашивала тебя что такое лицензия и зачем она нужна, а ты с радостью отвечал на ёё вопросы'
				if gender == 1:
					text = 'Мент пристально посмотрел сначала на лицензию, а потом на некомальчика. <b>"Ладно, всё в полном порядке. Извините за беспокойство"</b> – было сказано с некоторым разочарованием. ' + cnam + ' весь оставшийся день расспрашивал тебя что такое лицензия и зачем она нужна, а ты с радостью отвечал на его вопросы'
				bot.send_message(message.chat.id, text)
			...
			if licension < 0:
				...
				if d != 3:
					text = 'Невообразимо, но торговый автомат действительно выдал косяк. Вы с кайфом раскумарились, а потом [ДАННЫЕ УДАЛЕНЫ]. ' + cnam + ' определённо стала доверять тебе больше после этого дня\n\n+ 1 Доверия 💞\nНастроение повышено 🥰'
					if gender == 1:
						text = 'Невообразимо, но торговый автомат действительно выдал косяк. Вы с кайфом раскумарились, а потом [ДАННЫЕ УДАЛЕНЫ]. ' + cnam + ' определённо стал доверять тебе больше после этого дня\n\n+ 1 Доверия 💞\nНастроение повышено 🥰'
					rep += 1
			...
			if days < 4:
				...
				if days == 4:
					txt = 'Поздравляем с первым днем на новой должности! В твои обязанности входит проверять документы работников и либо пропускать их, либо слать нахуй. Подробнее можно прочитать в руководстве 📕. Помни, твоя зарплата зависит от количества правильных решений'
				else:
					txt = 'Ты пришел на рабочее место и готов принимать первых работников завода. Старая работа за станком определённо была лучше этой хуйни'
			...
			gtxt = ' со своей кошкоженой '
			if gender == 1:
				gtxt = ' со своим кошкомужем '
			markup = types.InlineKeyboardMarkup()
			...
			base_text = ['\n\n<b>Улучшить</b>  —  10 💰\nНемного интерьера 🏠\n<i>Эффект:  +2 доверия 💞</i>',
			'\n\n<b>Улучшить</b>  —  30 💰\nВысокотехнологичный верстак 🏠\n<i>Эффект:  Позволяет создавать предметы 🛠</i>',
			'\n\n<b>Улучшить</b>  —  50 💰\nНемного искусства 🏠\n<i>Эффект:  Настроение падает медленнее 😺</i>',
			'\n\n<b>Улучшить</b>  —  70 💰\nТелевизор со встроенным флексаиром 🏠\n<i>Эффект:  +4 доверия 💞</i>',
			'\n\n<b>Улучшить</b>  —  90 💰\nСтол с бухлом 🏠\n<i>Эффект:  Событие с Капитаном Морганом всегда даёт плюс доверие 💞</i>',
			'\n\n<b>Улучшить</b>  —  120 💰\nВсратый туалет 🏠\n<i>Эффект:  +6 доверия 💞</i>',
			'\n\n<b>Улучшить</b>  —  150 💰\nНе менее всратая кухня 🏠\n<i>Эффект:  +1 к получаемому вискасу 🍫</i>',
			...
			if coins < cost:
				bot.send_message(message.chat.id, 'А деньги где')
				return
			text = cnam + ' не очень понимает что это и зачем оно нужно, но ей понравилось'
			if gender == 1:
				text = cnam + ' не очень понимает что это и зачем оно нужно, но ему понравилось'
			bot.send_message(message.chat.id, text)
			...
			if d == 1:
				text = 'Внутри оказалась бездомная некодевочка. Как только ты открыл коробку, она моментально набросилась на твою, издавая шипящие звуки. К сожалению, ушло время чтобы их разнять. ' + cnam + ' обиделась на тебя за это\n\n– 2 Доверия 💞'
				if gender == 1:
					text = 'Внутри оказалась бездомная некодевочка. Как только ты открыл коробку, она моментально набросилась на твоего некомальчика, издавая шипящие звуки. К сожалению, ушло время чтобы их разнять. ' + cnam + ' обиделся на тебя за это\n\n– 2 Доверия 💞'
				rep = rep - 2
			...
			cursor.execute(f"UPDATE neko SET car = TRUE, coins = {coins} WHERE id = {message.from_user.id}")
				...
				if item_name == 'антипохмелин' and inventory['antipohmelin'] > 0:
					...
					if bone_automate:
						...
						text = cnam + ' определённо полюбит новое оружие 🦴'
						bot.send_message(message.chat.id, text)
						bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEGHbFjTQM_ocljKc2-O9GAUVL7Nb60TgACSQ8AAt46WUgVZwAB2AjTbT8qBA')
					...
					text = cnam + ', выпив содержимое банки, почуствовала в себе силу, способную свернуть горы. У неё появилось новое умение, выбери какой навык заменить на случайный:\n\n' + sktxt1 + '\n' + sktxt2
					if gender == 1:
						text = cnam + ', выпив содержимое банки, почуствовал в себе силу, способную свернуть горы. У него появилось новое умение, выбери какой навык заменить на случайный:\n\n' + sktxt1 + '\n' + sktxt2
					...
					inventory['gender_changer'] -= 1
					text = cnam + ' чувствует себя странно...'
					bot.send_message(message.chat.id,text)
					if gender == 0:
						photka = random.choice(trap_photos)
						while True:
					...
					text = cnam + ' с удовольствием будет носить эти обноски с ближайшей помойки'
					bot.send_message(message.chat.id, text)
					bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEGHbFjTQM_ocljKc2-O9GAUVL7Nb60TgACSQ8AAt46WUgVZwAB2AjTbT8qBA')
					...
					txt = 'Внутри коробки вы нашли странную вещь. ' + cnam + ', однако, сочла её красивой и захотела надеть. Выбери какой предмет гардероба заменить:\n\n' + item_list[item_one][0] + '\n\n' + item_list[item_two][0]
					if gender == 1:
						txt = 'Внутри коробки вы нашли странную вещь. ' + cnam + ', однако, сочёл её красивой и захотел надеть. Выбери какой предмет гардероба заменить:\n\n' + item_list[item_one][0] + '\n\n' + item_list[item_two][0]
				...
				if item_name == 'штаны за 40 гривень' and coins >= 40:
					coins -= 40
					inventory['armor1'] += 1
					bot.send_message(message.chat.id, cnam + ' определённо полюбит эту странную хуйню')
			...
			try:
				...
				if rep < REP_ARENA:
					...
					return
				if coins < 10:
					bot.send_message(message.chat.id, 'Ты слишком бомжара ебаный для этого')
					return
				if message.reply_to_message is None:
				...
				gender = data[0]
				chel = data[1]
				edit_text = '<a href="tg://user?id='+str(idk)+'">'+str(chel)+'</a>, твоей некодевочке бросили вызов! Приймешь или зассал?'
				if gender == 1:
					edit_text = '<a href="tg://user?id='+str(idk)+'">'+str(chel)+'</a>, твоему некомальчику бросили вызов! Приймешь или зассал?'
				keyboard = arena_init_keyboard(message.from_user.id)
				m = bot.send_message(message.chat.id, text = f'Ставка 10 💰',reply_markup=keyboard)
			...
			text = 'Очевидно, бои некочанов нелегальны, поэтому опустим лишние подробности. Обязательным условием проведения боя является ставка, часть которой организаторы забирают себе. Лучшие некочаны арены:\n\n'
			...
			text = text + '\n\n<i>Напиши <u>бой</u> чтобы бросить вызов ответом на сообщение</i>'
			bot.send_photo(message.chat.id, photo = 'AgACAgIAAx0CZQN7rQACqQpizlI_XJiCwrzrSCYH47ZtXq9cCwACfLwxG0_FeEqC6_m0bVQSoQEAAwIAA3MAAykE',caption = text)
		elif cmd == 'лицензия':
				markup = types.InlineKeyboardMarkup()
				...
				if coins < cost:
					...
					return
				if licension >= 0:
					bot.send_message(message.chat.id, 'Нахуя продлевать лицензию если одна действует')
					return
				coins = coins - cost 
				f = create_licension(bot,phot,photo_design,message.from_user.first_name,gender)
			...
			if dungeon_kd > 0:
				...
				return
			if rep < REP_DUNGEON:
				text = cnam + ' отказалась входить, не стоит её заставлять'
				if gender == 1:
					text = cnam + ' отказался входить, не стоит его заставлять'
				bot.send_message(message.chat.id,text)
			...
			text = cnam + ' сразу же почуствовала прохладу и сырость, а её нога вступила во что-то мокрое. Да это же огромная пещера! Исходящее отовсюду разноцветное свечение прогоняет темноту даже с самых отдалённых уголков\n\n' + '0 💰   0 🍫   0 ⚡️   0 🍼\n'+maptxt
			if gender == 1:
				text = cnam + ' сразу же почуствовал прохладу и сырость, а его нога вступила во что-то мокрое. Да это же огромная пещера! Исходящее отовсюду разноцветное свечение прогоняет темноту даже с самых отдалённых уголков\n\n' + '0 💰   0 🍫   0 ⚡️   0 🍼\n'+maptxt
			...
			m = bot.send_message(message.chat.id, text = f'Идёт набор в тиму для легендарной пизделки с боссом\n\nВход 1 ⚡️, {REP_BOSS} 💞\n<b>Игроков: 1</b>',reply_markup=keyboard)
			...
			text = f'Это военный объект, но благодаря связям тебе удалось попасть сюда. Место, куда ведёт портал, принятно называть LGBT миром, и про него практически ничего неизвестно. Последняя исследовательская экспедиция считается пропавшей без вести\n\nУспешных забегов:  {dungeon_raids} 🏳️‍🌈'
			markup = types.InlineKeyboardMarkup()
			...
			text = 'Чтобы не платить деньги, казино Некославии часто предлагают в качестве выигрыша всякий мусор. Это, базирующееся в глубинах подвала твоего дома, не оказалось исключением. Способов проебать зарплату здесь не так уж и много\n\n<b>Слоты 🎰</b>\nПуск за 10 денег. Запусти слоты и попытай удачу если не зассал\n<b>Покер 🃏</b>\nИгра от 20 денег. Сыграй с такими же полупокерами как ты'
			markup = types.InlineKeyboardMarkup()
